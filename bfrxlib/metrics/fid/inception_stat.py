...
def inception_fid_stat_folder():
    ...
    parser.add_argument('-i', '--folder', type=str, help='Path to the folder.', default='datasets/ffhq/ffhq_256/')
    # naming rule: _, name, size = args.fid_stats.replace('.pth', '').split('_')
    ...
    _, name, size = args.fid_stats.replace('.pth', '').split('_')
    state_dict = {'name': name, 'size': size, 'mean': sample_mean, 'cov': sample_cov}
