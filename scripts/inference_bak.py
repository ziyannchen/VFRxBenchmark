...
def main(root_path):
    ...
    # ------------------ set up dataloaders ----------------------
    dataset_props = opt['datasets'].pop('props')
    dataset_opts = opt['datasets'].values()
    test_loaders = []
    for dataset_opt in dataset_opts:
        dataset_opt.update(dataset_props)
    ...
    for dataloader, dataset_opt in zip(test_loaders, dataset_opts):
        ...
        for data in dataloader:
            ...
            if 'video' in model.inference_type:
                ...
                face_helper = FaceRestoreHelper(device=device, face_size=dataset_props['face_size'], **opt['helper'])
